    ...
    for i in flightinfo:
        if i['depStsCd'] == 'AIR' or i['latestTailNr'] == {} or i['alnCd'] != 'CZ':
            ...
            if tempofpl == {}:
                ...
                if DBprocess.queryoData(tempofpl) == 0:
                    try:
                        tempo = base64ToString(tempofpl['ofpText'])
                        # TODO add write to file with config
                    except Exception:
                        config.logger.warning('wrong with base64 fpl transform!fltNr=%s,depCd=%s,arvCd=%s'%(i['fltNr'],i['latestDepArpCd'],i['latestArvArpCd']),exc_info=True)
                        continue
                    try:
                        detail = ofptextprocess(tempo)
                    except Exception:
                        config.logger.debug('ofpprocess goes wrong!fltNr=%s,depCd=%s,arvCd=%s'%(i['fltNr'],i['latestDepArpCd'],i['latestArvArpCd']),exc_info=True)
                        continue
                    DBprocess.insertData(tempofpl, detail,config.logger)
                else:
